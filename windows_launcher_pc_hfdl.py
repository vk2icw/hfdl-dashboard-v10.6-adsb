...
class Launcher(tk.Tk):
    def __init__(self) -> None:
        super().__init__()
        self.title(f"{APP_NAME} — PC-HFDL Log Input")
    ...
    def build_ui(self) -> None:
        ...
        ttk.Label(root, text="Windows launcher with dumphfdl UDP and PC-HFDL logfile input.").pack(anchor="w", pady=(2, 14))
        ...
        ttk.Label(pc, text="PC-HFDL still receives audio through VAC. The launcher reads new decoded records from the .txt log and forwards them to the dashboard.", foreground="#555").pack(anchor="w", pady=(8, 0))
        ...
        ttk.Checkbutton(options, text="Minimise to notification area while running", variable=self.values["minimize_to_tray"]).pack(anchor="w", pady=(4, 0))
        ...
        self.set_detail("Use dumphfdl UDP directly, or enable PC-HFDL Log Input and select the folder containing PC-HFDL .txt logs.")
    ...
    def load_settings(self) -> None:
        ...
        for key, value in self.values.items():
            if key in data: value.set(data[key])
    ...
    def save_settings(self, quiet: bool = False) -> None:
        ...
        if not quiet: messagebox.showinfo(APP_NAME, "Settings saved.")
    ...
    def choose_database(self) -> None:
        ...
        if filename: self.values["database_path"].set(filename)
    ...
    def refresh_pc_controls(self) -> None:
        ...
        if hasattr(self, "pc_start") and not self.pc_running(): self.pc_start.configure(state=state)
    ...
    def start_pc_hfdl(self, quiet: bool = False) -> bool:
        if self.pc_running(): return True
        if not self.values["pc_hfdl_enabled"].get(): return False
        path = Path(self.values["pc_hfdl_log_path"].get().strip()).expanduser()
        if not path.exists():
            if not quiet: messagebox.showerror(APP_NAME, f"PC-HFDL log path does not exist:\n\n{path}")
            return False
        try: port = int(self.values["udp_port"].get())
        except ValueError: return False
        self.pc_stop.clear(); self.pc_count = 0; self.pc_count_text.set("0 messages"); self.pc_status.set("Starting…")
        self.save_settings(quiet=True)

        def worker() -> None:
            watcher = PCHFDLWatcher([path], self.udp_target(), port, self.values["pc_hfdl_pattern"].get().strip() or "*.txt", True)
    ...
    def stop_pc_hfdl(self) -> None:
        self.pc_stop.set()
        if self.pc_thread and self.pc_thread.is_alive(): self.pc_thread.join(timeout=2)
    ...
    def validate(self):
        ...
        if bool(user) != bool(password): messagebox.showerror(APP_NAME, "Enter both username and password, or leave both blank."); return None
    ...
    def server_command(self):
        ...
        if script.exists(): return [sys.executable, str(script)]
    ...
    def start_server(self) -> None:
        ...
        self.set_detail(f"UDP listener: {udp_ip}:{udp_port}\nWeb listener: {web_ip}:{web_port}\nDatabase: {db_path}\nPC-HFDL input: {'enabled' if self.values['pc_hfdl_enabled'].get() else 'disabled'}")
        if self.values["pc_hfdl_enabled"].get() and self.values["pc_hfdl_autostart"].get(): self.after(900, lambda: self.start_pc_hfdl(quiet=True))
    ...
    def ready(self) -> None:
        ...
        if self.values["open_browser"].get(): self.open_dashboard()
    ...
    def open_dashboard(self) -> None: webbrowser.open_new_tab(self.dashboard_url() + f"&cb={int(time.time())}")
    ...
    def poll_process(self) -> None:
        if self.process and self.process.poll() is not None:
            code = self.process.returncode; self.process = None; self.stop_pc_hfdl(); self.start_btn.configure(state="normal"); self.stop_btn.configure(state="disabled"); self.status.set(f"Stopped unexpectedly (exit code {code})")
        self.after(1000, self.poll_process)
    ...
    def configure_firewall(self) -> None:
        ...
        try:
            ...
            if result.returncode == 0: self.values["firewall_port"].set(str(port)); self.save_settings(quiet=True); messagebox.showinfo(APP_NAME, f"Firewall updated for UDP {port}.")
        except Exception: messagebox.showerror(APP_NAME, "Firewall update failed.")
    ...
    def on_close(self) -> None:
        ...
        if running and self.values["minimize_to_tray"].get(): self.withdraw(); return
        if running and not messagebox.askyesno(APP_NAME, "Stop the dashboard and PC-HFDL input, then exit?"): return
