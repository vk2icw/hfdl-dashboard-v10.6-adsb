...
# Add an explicit full-photo action beside the existing photographer/source text if the block is present.
needle = "let photographer=photo.photographer||'Unknown photographer';"
if needle not in text:
    raise SystemExit('Expected Planespotters credit block was not found; app.py may have changed.')
text = text.replace(needle, needle + "\n  credit.innerHTML=`Photo: ${photographer}<div class=\\\"photo-actions\\\"><a href=\\\"${photo.link}\\\" target=\\\"_blank\\\" rel=\\\"noopener noreferrer\\\">Open original photograph</a></div>`;", 1)
